...
const DOWNLOAD_DIR = path.join(os.homedir(), '.decrypt-log-mcp', 'downloads')
...
async function decryptBuffer(cipherData: Uint8Array, fileName: string, platform: string): Promise<{ files: Array<{ name: string; data: Buffer }> }> {
...
server.tool(
  'decrypt-log',
  '通过 URL 解密 CamScanner 加密日志。自动下载、解密并保存到本地。支持 iOS 和 Android 日志。',
  {
    log_url: z.string().describe('加密日志文件的 URL 链接'),
    platform: z.enum(['iOS', 'Android']).optional().default('iOS').describe('日志平台，默认 iOS'),
    output_dir: z.string().optional().describe('解密后文件的保存目录（绝对路径），默认 ~/.decrypt-log-mcp/downloads/'),
  },
  async ({ log_url, platform, output_dir }) => {
    ...
    try {
      ...
      // Save files
      if (files.length === 1) {
        const savePath = path.join(downloadPath, 'decrypt_' + filename)
        fs.writeFileSync(savePath, files[0].data)
        ...
        const savePath = path.join(downloadPath, 'decrypt_' + filename.replace(/\.[^.]+$/, '') + '.zip')
...
server.tool(
  'decrypt-log-file',
  '解密本地加密日志文件。传入本地文件路径，直接解密并保存结果。',
  {
    file_path: z.string().describe('本地加密日志文件的绝对路径'),
    platform: z.enum(['iOS', 'Android']).optional().default('iOS').describe('日志平台，默认 iOS'),
    output_dir: z.string().optional().describe('解密后文件的保存目录（绝对路径），默认 ~/.decrypt-log-mcp/downloads/'),
  },
  async ({ file_path, platform, output_dir }) => {
    ...
    try {
      ...
      const cipherData = new Uint8Array(fs.readFileSync(file_path))
...
server.tool(
  ...
  '列出已解密的日志文件。',
  {
    output_dir: z.string().optional().describe('查看指定目录，默认 ~/.decrypt-log-mcp/downloads/'),
  },
  async ({ output_dir }) => {
    const downloadPath = output_dir || DOWNLOAD_DIR
